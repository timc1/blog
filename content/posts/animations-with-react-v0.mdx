---
date: '2019-02-04'
title: 'Tracking User Scrolling With React Hooks'
short_name: 'React'
scope: 'Experiment'
seo_description: 'Web Animations, React Animations, User Interface Design,'
---

React makes building UIs really fun 🤗
...
3. Use our hook to trigger state updates
4. Animating our scroll bar
...
### Use our new hook to trigger animations
...
Now every time an element is in view or has left, our hook will trigger our
callback. Let's write the logic for our callback. Let's replace that console.log
from our demo.js file and replace it:
...
useIntersectionObserver({
  ...
  callback: ({ isIntersecting, target }) => {
    console.log('isIntersecting', isIntersecting, target)
    const sectionData = target.getAttribute('data-section-id')
    if (isIntersecting) {
      //dispatchScrollingNav({
      //  type: sectionData
      //});
    ...
    console.log('current', currentSection, 'previous', previousSection)
  },
  options: {
    root: navRef.current,
  },
...
### Animate a progress bar on scroll

Let's animate our `<li className="scaler" />` now. This will scale
proportionally to the amount of content we have left to scroll on the screen.

In our demo.js file, we'll pass `refs` to mark the **beginningContent** and the
**endContent** to set a boundary to allow our `<Nav />` to calculate the scaling
amount.
...
and in our nav.js file we'll pass the two new props down:

```js
export default React.forwardRef(function Nav({ sections, beginningContent, endContent }, ref) {
// ...
```

Now that we have a reference to the beginning and end content, we can use the
`useEffect` hook to setup calculations when the Nav component mounts. In our
nav.js file:
...
export default React.forwardRef(function Nav(
  { sections, beginningContent, endContent },
  ref
) {
  // Setup a reference to the div we're going to animate
  const scalerRef = useRef(null)

  // This effect will only fire once after a component mounts, and before it unmounts.
  useEffect(() => {
    const {
      top: topOfBeginning,
    ...
    const scrollEvent = () => {
      ...
      // transform our <li className="scaler" />
      ...
      <Items>
        {sections.map(section => (
          <>
            ...
            <li ref={scalerRef} className="scaler" aria-hidden="" />
          </>
        ))}
      </Items>
    </Aside>
...
Now when we scroll the page, we should see our scaler element transforming like
this:
