...
The fill demo is available to play with live
...
2. Create a hook to track an when element intersections with the viewport or
   another element
...
To do this, we will not be using the IntersectionObserver hook, but a `scroll`
event listener.

The basic idea is:

1. Create a function calculates how much content is left to scroll.
2. Set the style of the `<li className='scaler' />` based on that percentage.
